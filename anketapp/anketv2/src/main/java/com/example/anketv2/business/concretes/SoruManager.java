...
public class SoruManager implements SoruService {
	...
	@Autowired
    private YazmaliSoruRepository yazmaliSoruRepository;
	...
	public ÇoktanSeçmeliSoru saveSeçmeliSoru(saveÇoktanSeçmeliSoruRequest saveÇoktanSeçmeliSoruRequest) {
		...
		çoktanSeçmeliSoru.setSoruMetni(saveÇoktanSeçmeliSoruRequest.getSoruMetni());
    ...
    public BooleanSoru saveBooleanSoru(saveBooleanSoruRequest saveBooleanSoruRequest) {
        ...
        booleanSoru.setSoruMetni(saveBooleanSoruRequest.getSoruMetni());
    ...
    public YazmaliSoru saveYazmaliSoru(saveYazmaliSoruRequest saveYazmaliSoruRequest) {
        YazmaliSoru yazmaliSoru = modelMapperService.forRequest().map(saveYazmaliSoruRequest, YazmaliSoru.class);
        yazmaliSoru.setSoruMetni(saveYazmaliSoruRequest.getSoruMetni());
        return yazmaliSoruRepository.save(yazmaliSoru);
    }

    @Override
    public List<GetallYazmaliSoruResponse> getallYazmaliSoruResponses() {
        List<YazmaliSoru> yazmaliSorus = yazmaliSoruRepository.findAll();
        List<GetallYazmaliSoruResponse> yazmaliSoruResponses = yazmaliSorus.stream()
                .map(yazmaliSoru -> this.modelMapperService.forResponse()
                        .map(yazmaliSoru, GetallYazmaliSoruResponse.class)).collect(Collectors.toList());
        return yazmaliSoruResponses;
    }
